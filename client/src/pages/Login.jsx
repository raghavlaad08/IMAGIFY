...
const Login = () => {
  ...
  // Use the login function from context instead of manually setting token
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (state === "login") {
          // Use the login function for login
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 m-auto items-start p-8 py-12 w-80 sm:w-[352px] text-gray-500 rounded-lg shadow-xl border border-gray-200 bg-white">
      ...
      <button 
        type='submit' 
        disabled={loading}
        className={`${loading ? 'bg-purple-400' : 'bg-purple-700 hover:bg-purple-800'} transition-all text-white w-full py-2 rounded-md cursor-pointer disabled:cursor-not-allowed`}
      >
        {loading 
